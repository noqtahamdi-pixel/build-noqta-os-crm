...
export default async function Page() {
  ...
  const [{ data: styles, error: stylesError }, { data: categories, error: categoriesError }, { data: products, error: productsError }] = await Promise.all([
  ...
  return <Shell user={{ email: user.email ?? '', role, mfa: false }} styles={styles ?? []} categories={categories ?? []} subcategories={subcategories ?? []} products={products ?? []} lastSync={null} dataError={stylesError?.message ?? categoriesError?.message ?? productsError?.message ?? subcategoriesError?.message ?? null} />
}
